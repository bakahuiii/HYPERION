import { CheckCircle2, ChevronDown, ChevronRight, FilePenLine, HardDrive, Palette, RefreshCw, SlidersHorizontal, Trash2, X } from 'lucide-react'
...
export function OptionsView({ settings, onSettingsChange, onAppearance, personCount, questCount, onClearPeople, onClearQuests }: OptionsViewProps) {
  ...
  return <div className="options-view page-width">
    ...
    <section className="options-section"><div className="options-heading"><div><SlidersHorizontal size={18} /><div><h3>提炼策略</h3><p>这里决定分析范围、时效和筛选偏好；下面的模型提示词只细化各工作流的表达。并发只作用于不同会话，同一会话始终按时间顺序处理。</p></div></div></div><div className="ai-controls"><label><span>分析模式</span><select value={settings.mode} onChange={(event) => updateSettings({ mode: event.target.value as AiSettings['mode'] })}><option value="balanced">明确事项与提醒</option><option value="action">行动优先</option><option value="planning">长期规划</option><option value="review">复盘与整理</option></select></label><label><span>历史事项范围</span><select value={settings.recencyPolicy} onChange={(event) => updateSettings({ recencyPolicy: event.target.value as AiSettings['recencyPolicy'] })}><option value="strict">严格：更快淘汰短时事项</option><option value="balanced">平衡：保留仍有效安排</option><option value="broad">宽泛：更多历史长期事项</option></select></label><label><span>自动更新</span><select value={settings.intervalHours} onChange={(event) => updateSettings({ intervalHours: Number(event.target.value) })}><option value="24">每 24 小时</option><option value="48">每 48 小时</option><option value="72">每 72 小时</option><option value="168">每 7 天</option></select></label><label><span>并发会话</span><select value={normalizeAiConcurrency(settings.concurrency)} onChange={(event) => updateSettings({ concurrency: Number(event.target.value) })}>{AI_CONCURRENCY_OPTIONS.map((value) => <option key={value} value={value}>{value} 个会话</option>)}</select></label><label className="auto-switch"><input type="checkbox" checked={settings.autoEnabled} onChange={(event) => updateSettings({ autoEnabled: event.target.checked })} /><span>自动提炼未分析记录</span></label></div><label className="ai-instructions"><span>任务筛选偏好 <small>自动保存，下一次提炼立即按此要求执行</small></span><textarea value={settings.instructions} onChange={(event) => updateSettings({ instructions: event.target.value })} rows={4} placeholder="例如：优先保留约会、约见、预约、报名、课程、截止和需回复事项；日期不明确时留空。" /></label><div className="feedback-memory"><div><strong>候选磨合记录</strong><span>已保存 {settings.feedback?.length ?? 0} 条保留或忽略反馈；下一次提炼会参考最近的偏好。</span></div><div className="feedback-actions"><button type="button" className="secondary-button" onClick={() => setFeedbackOpen(true)} disabled={!settings.feedback?.length}>查看记录</button><button type="button" className="icon-button" title="清空全部磨合记录" aria-label="清空全部磨合记录" onClick={() => updateSettings({ feedback: [] })} disabled={!settings.feedback?.length}><Trash2 size={15} /></button></div></div></section>
