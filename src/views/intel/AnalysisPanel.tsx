      ...
      {open && <>
        <div className="intel-analysis-brief">
          ...
          <div className="intel-workflow-picker" role="group" aria-label="提炼内容">
            ...
            <button type="button" className="text-button" onClick={() => onTargetsChange({ tasks: true, people: true })} disabled={aiBusy || (analysisTargets.tasks && analysisTargets.people)}>全选</button>
          </div>
          ...
          <span>{analysisConversationId
            ...
            : timelineMode === 'last-chat'
              ...
              : `严格窗口内有 ${formatCount(analysisMessageCount)} 条消息，来自 ${analysisConversationCount} 个会话；窗口外消息不会发送。`}</span>
        </div>
