export const APP_STORAGE_SCHEMA = 'theia-app-state'
...
export function unwrapAppStorage<T = unknown>(value: unknown): UnwrappedAppStorage<T> {
  if (!isRecord(value) || value.schema !== APP_STORAGE_SCHEMA) {
  ...
  if (schemaVersion > APP_STORAGE_SCHEMA_VERSION) {
    throw new Error(`本地数据由更高版本的 THEIA 创建（schema ${schemaVersion}），当前版本最高支持 ${APP_STORAGE_SCHEMA_VERSION}`)
  }
  if (!Object.hasOwn(value, 'data')) throw new Error('本地数据缺少 data 字段')
  return { data: value.data as T, schemaVersion }
}
