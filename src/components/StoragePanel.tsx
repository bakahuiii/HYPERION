...
export function StoragePanel({ open, storage, message, personCount, questCount, onToggle, onRefresh, onClearPeople, onClearQuests }: StoragePanelProps) {
  ...
  const archiveIntegrity = health?.archive.integrity
  const archiveWarning = health?.archive.migration.state === 'failed' || (archiveIntegrity?.status !== undefined && !['verified', 'empty'].includes(archiveIntegrity.status))
  return (
    <section className={`options-section storage-section ${open ? 'is-open' : ''}`}>
      <div className="options-heading"><div><HardDrive size={18} /><div><h3>数据与存储</h3><p>{open ? '所有持久化文件统一位于当前 HYPERION 工作目录；这里可确认版本、恢复状态、位置与占用。' : '已折叠；展开后查看本地数据的位置、用途和健康状态。'}</p></div></div><div className="storage-actions">{open && <button type="button" className="icon-button" title="刷新存储概览" aria-label="刷新存储概览" onClick={onRefresh}><RefreshCw size={15} /></button>}<button type="button" className="icon-button" title={open ? '折叠数据与存储' : '展开数据与存储'} aria-label={open ? '折叠数据与存储' : '展开数据与存储'} aria-expanded={open} onClick={onToggle}>{open ? <ChevronDown size={17} /> : <ChevronRight size={17} />}</button></div></div>
      {open && <div className="storage-panel">
        ...
        {storage && <>
          ...
          {health && <div className="storage-health-grid" aria-label="数据健康状态">
            ...
            <article className={archiveWarning ? 'is-warning' : ''}><HardDrive size={17} /><div><strong>{health.archive.recordCount.toLocaleString('zh-CN')} 条 · {health.archive.segmentCount} 个归档段</strong><span>{health.archive.storageEngine} · {formatTime(health.archive.updatedAt)} · {archiveIntegrity?.status === 'verified' ? 'SHA-256 已验证' : archiveIntegrity?.status === 'empty' ? '等待首次归档' : archiveIntegrity?.status === 'recovered-unindexed' ? `已从分段恢复索引（${archiveIntegrity.unindexedSegmentCount} 段待写回）` : archiveIntegrity?.status === 'legacy-pending-migration' ? '旧归档等待迁移' : '完整性尚未验证'}</span></div></article>
            ...
            <article><RotateCcw size={17} /><div><strong>{health.sharedState.rollbackBackups.length} 份迁移回滚备份</strong><span>关闭 HYPERION 后运行 <code>{health.rollbackCommand}</code></span></div></article>
          </div>}
