...
HYPERION 的设备时间线数据只接收独立 Android 或 Windows 项目 **SELENE** 的输出。SELENE
...
在 HYPERION 中连接 SELENE 选择的父目录。每次采集都会新建一个不可变子目录：
...
HYPERION 会递归扫描这些子目录，仅接受以下 UTF-8 JSON 外层结构。旧伴生程序
格式会被明确拒绝，不再作为兼容输入。
...
从不回写、合并或删除旧快照。若两个采集窗口存在重叠，HYPERION 会按稳定的
...
HYPERION 本地保存。发送给模型的是粗粒度投影：坐标、地址、地理哈希及类似
