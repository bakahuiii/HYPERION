...
[简体中文](DEVELOPER_GUIDE.md) | [English](DEVELOPER_GUIDE.en.md) | [项目终极目标](PROJECT_VISION.md)
...
这是 THEIA 的最高产品目标和架构约束。当前任务图、聊天记录导入、人物卡、行程、地图、模型提炼和存储迁移都只是为这一目标铺设的数据与交互基础设施。项目不以“管理得更高效”“从聊天中生成更多任务”或“给人打分”为成功标准。

任何重要功能、数据字段或模型流程在进入实现前，都必须回答：

1. 它连接了哪些跨时间证据，而不是只制造一个当下结论？
2. 它如何帮助用户理解变化、延续、转折及其上下文？
3. 它能否区分原始观察、模型推断、用户反思和后续修正？
4. 用户能否查看来源、纠正结论、撤销变更并带走自己的数据？
5. 如果它不能帮助回答“我是如何成为今天的我的，又正在成为怎样的人”，是否应该进入核心产品？

因此，工程实现必须保留时间、来源、发言方向和版本；推断不得伪装成事实；人格刻画不得把零散瞬时状态拼成确定性标签；建议不得演变为预测、评分、操控或道德裁决。更完整的目标与设计原则见 [项目终极目标](PROJECT_VISION.md)。

### 1.2 当前工程边界

THEIA 当前是单用户、本地优先的个人纵向研究应用。它的信任边界是：用户主动提供导出文件，本地前端负责解析和选择，本地 Node.js 服务负责持久化、受控媒体代理及模型转发，外部模型只接收用户主动提交的范围。
