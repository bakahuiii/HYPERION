...
429、502、503、504、524、网络错误和超时只增加当前通道的失败计数并触发 1–15 秒指数冷却。客户端随后重试 `/api/ai/analyze` 时会重新申请 lease，因此可能切到另一通道；服务端不会把一个请求广播到所有通道。`/api/ai/status` 的顶层 `configured` 表示“至少一条启用且有效的通道存在”，不能只反映主通道。
