...
- 可写入日记、每日状态快照与导入 AI 对话；用户主动选择“自我”分析后，结果会保留精确来源，不把推断写成事实或诊断。
- 大型聊天归档使用追加式 gzip JSONL 分段、完整性校验与恢复路径，减少全量重写；界面对会话浏览与大文件导入做了分页和 Worker 优化。
- 多通道提炼保留稳定重试和可观测性；多模型裁决的数据格式已经建立，但本版本不会将同一份聊天自动发送到多个模型。
