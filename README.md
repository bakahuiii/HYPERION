...
> 当前源码版本：`0.2.0`。现有 Windows x64 便携版仍为 `0.1.1`；便携版自带 Electron 与 Node.js 运行时，无需另行安装 Node.js，任务、设置、聊天归档和日志保存在 `%APPDATA%\\THEIA`。重要数据请定期备份。

## 0.2.0 版本重点

- 模型配置升级为多通道池：每条 API 可独立设置地址、Key、模型、模式、启停状态和并发容量。
- 不同会话可并行分配到多条健康通道；429、502、503、504、524、网络错误和超时只冷却故障通道。
- 私聊的任务与人物证据合并为一次模型输入，减少重复上传、输入 token 和等待时间。
- 全局会话并发可调至 1–32；单通道容量为 1–8，同一会话的连续片段仍严格串行。
- 增加按任务写入的供应商尝试、用量和失败阶段日志，并补充本地假服务集成测试。
- 修复分段记录范围在提示词构造阶段触发 `values.map is not a function`、导致请求尚未到达 API 就批量失败的问题。

完整变化、升级注意事项和验证结果见 [0.2.0 更新报告](docs/RELEASE_NOTES.md)。
